...
import { TAG_DB_MIRRORS, type TagDbMirror } from '@/services/tags/tagDb'
import { TAG_COUNT_MIRRORS, type TagCountMirror } from '@/services/tags/tagCount'
import { TAG_WIKI_MIRRORS, type TagWikiMirror } from '@/services/tags/tagWiki'
...
export type DblClickAction = 'search' | 'searchNewTab' | 'clearSearch' | 'toggleEdit' | 'openSearchPopup' | 'none'
...
const SETTING_VALIDATORS: Partial<{ [K in SettingKey]: (v: unknown) => boolean }> = {
  ...
  myTagsPanelZoom:        v => typeof v === 'number' && Number.isFinite(v) && v >= 50 && v <= 150,
...
function isValidButton(x: unknown): x is Button {
  ...
  if (o.kind === 'spacer') {
    return (o.mode === 'flex' || o.mode === 'fixed')
      && (o.width === undefined || (typeof o.width === 'number' && Number.isFinite(o.width) && o.width > 0))
  }
  return false
}
